...
namespace Task_Application.Repository
{
    public abstract class RepositoryBase<T> : ControllerBase, IRepository<T> where T : class, IEntity
    {
        ...
        public  IEnumerable<T> GetFile(CancellationToken ct)
        {
            return (IEnumerable<T>)_context.Set<T>().FromSqlRaw($"EXEC {ProcedureName} @choice='Get'").ToListAsync(cancellationToken: ct);
